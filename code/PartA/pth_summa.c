...
void scatterv(comm_t *c, int my_rank, int sender, int counts_send[], int disp[], int* buf_send, int* buf_recv, int count_recv, vec *send_type, vec *recv_type);
...
void *thread_func(void *arg){
  ...
  if (my_coor[1] == 0){
    ...
    scatterv(col_comm, my_coor[0], 0, counts_send[0], disp[0], &A[0][0], full_rows[0], M_sz[r]* MKN[1], &INT_TYPE, &INT_TYPE);
    scatterv(col_comm, my_coor[0], 0, counts_send[1], disp[1], &B[0][0], full_rows[1], K_sz[r]* MKN[2], &INT_TYPE, &INT_TYPE);
  }
  ...
  scatterv(row_comm, my_coor[1], 0, counts_send[0], disp[0], full_rows[0], local_A, K_sz[c], &A_vec[0], &A_vec[1]);
  scatterv(row_comm, my_coor[1], 0, counts_send[1], disp[1], full_rows[1], local_B, N_sz[c], &B_vec[0], &B_vec[1]);
  ...
  for (k = 0; k < grid_sz; k++){
    kk = K_sz[bz_idx(k, grid_sz)];
    if (my_coor[1] == k){ // sender of A
      for (i = 0; i < mm; i++){
        for (j = 0; j < kk; j++){
          temp_A[i*kk+j] = local_A[i*kk+j];
        }
      }
    }
    broadcast(row_comm, k, my_coor[1], mm*kk, temp_A);
    if (my_coor[0] == k){ // sender of B
      for (i = 0; i < kk; i++){
        for (j = 0; j < nn; j++){
          temp_B[i*nn+j] = local_B[i*nn+j];
        }
      }
    }
    broadcast(col_comm, k, my_coor[0], kk*nn, temp_B);
    
    for (int kkk = 0; kkk < kk; kkk++){
      for (i = 0; i < mm; i++){
        for (j = 0; j < nn; j++){
          local_C[i*nn+j] += temp_A[i*kk + kkk] * temp_B[kkk*nn + j];
        }
      }
    }
  }
...
int main(int argc, char* argv[]){
  if (argc != 3){
    printf("./pth_summa <num_threads> <input_filename>\n");
    exit(-1);
  }
  comm_sz = atoi(argv[1]);
...
void buildVectorTypes(int my_rank, int* MKN, int grid_sz, int* my_coor, int** MKN_sz, vec** ABC_vec){
  ...
  for (k = 0 ; k < 3; k++){
    ...
    // submatrix vec type
    ABC_vec[k][1] = (vec) {.block_count= MKN_sz[ABC_dim[k].r][i],.block_length= 1,.stride= MKN_sz[ABC_dim[k].c][j]};
  }
}
...
void gatherv(comm_t *c, int my_rank, int global_rank, int receiver, int counts_recv[], int disp[], int* buf_send, int* buf_recv, int count_send, vec *send_type, vec* recv_type){
  ...
  if (my_rank != 0){
    ...
    count = count_send * c->type->block_count * c->type->block_length;
  ...
  for (i = 1; i < c->N; i<<=1){
    ...
    if (my_rank % i == 0 && partner < my_rank) {
      ...
      if (my_rank != 0){
        ...
        count *= (c->type->block_count) * (c->type->block_length);
        disp_j *= (c->type->block_count) * (c->type->block_length);
